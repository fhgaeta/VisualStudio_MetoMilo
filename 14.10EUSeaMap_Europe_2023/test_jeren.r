#plot single area - Jæren 14.10 EUSeaMap_Europe_2023

#14.10EUSeaMap_Europe_2023
...
file_path_gdb <- "C:/Users/FEG/OneDrive - NIVA/METOMILO_OneDrive/GIS Data/Ecological components data/EUSeaMap_Europe_2023/EUSeaMap_2023.gdb"
file_path_gdb_local <- "C:/Users/FEG/Downloads/Downloads/EUSeaMap_2023.gdb"
...
species_list <- shp_reker %>%
  filter(species_no %in% c("Dypvannsreke")) %>%
  distinct(species_no) %>%
  pull(species_no) %>%
  as.character()
...
for (i in seq_along(species_list)) {
  ...
  shp <- shp_reker %>% filter(species_no == species)
  ...
  file_out <- paste0(local_folder, "14.9_", species, "_Jæren", ".csv")
