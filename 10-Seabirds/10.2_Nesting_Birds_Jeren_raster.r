...
ggsave(p, filename=paste0(folder_output_od, "Seabirds_Jeren_vectorial", shp_area_3$vannregion[1], ".png"),
...
if (!is.null(rasterized_birds)) {
  ...
  stop("Rasterization failed. Please check the input data.")
}

## Ensure the speciesnorwegianname field is treated as a factor
rasterized_birds_shp$speciesnorwegianname <- 
  as.factor(rasterized_birds_shp$speciesnorwegianname)
...
# Save the vectorial plot
ggsave(p3, filename=paste0(folder_output_od, "Seabirds_Jeren_raster_1000", shp_area_1$vannregion[1], ".png"),
       dpi=300, height=20, width=20, units="cm", bg="white")
