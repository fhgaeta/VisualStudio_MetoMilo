#plot single area - Hardanger 14.2 Bløttbunnskorallskog
...
ggplot() +
  ...
  theme_minimal() +
  labs(subtitle = "14.2 Bløtbunnskorallskog - Hardanger")
...
p1 <- ggplot() +
  ...
  guides(colour = guide_legend(ncol = 1, bycol = T)) +
  theme_minimal() +
  labs(subtitle = "14.2 Bløttbunnskorallskog - Hardanger")

# Save the vectorial plot for octocoral
ggsave(p1, filename = paste0(folder_output_od, "14.2 Bløtbunnskorallskog_Hardanger_vectorial_", shp_area_2$vannregion[1], ".png"),
...
for (species_to_rasterize in species_list) {
 ...
 shp_octoral_inters_selection <- shp_octoral_inters %>%
  ...
  ggsave(
    ...
    filename = paste0(
      folder_output_od, 
      "14.2 Bløtbunnskorallskog_Hardanger_raster_100_", 
