#plot single area 3 - Jæren. 14.2 Bløtbunnskorallskog
# Missing only input from Astrid regarding the species to be included in the analysis. and some issue with the output path
...
plot_title <- paste0("Vannregion: ", shp_area_1$vannregion[1])
 ...
 # Filter the shapefile for the current species
  shp_smb_proj_inters_filtered <- shp_smb_proj_inters %>%
    filter(sarbarthabitat %in% c("Hydrozoa hage",
...
p1 <- ggplot() +
  ...
  geom_sf(data=shp_smb_proj_inters_filtered, aes(colour=sarbarthabitat)) +
  coord_sf() +
...
ggsave(p1, filename=paste0(folder_output_od, "14.2 Bløtbunnskorallskog_Jeren_vectorial", shp_area_1$vannregion[1], ".png"),
...
for (species_to_rasterize in species_list) {
  ...
  plot <- ggplot() +
    ...
    scale_fill_discrete(name = "14.2 Bløtbunnskorallskog art ") +
    geom_point(data = df, aes(x = x, y = y), color = "red")

  # Define the folder path
  folder_output_od <- paste0("C:/Users/FEG/OneDrive - NIVA/METOMILO_OneDrive/Output_final/")
  ...
  ggsave(
    ...
    filename = paste0(
      folder_output_od,
       "14.2 Bløtbunnskorallskog_Jeren_raster_100_",
