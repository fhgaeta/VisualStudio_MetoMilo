...
#plot the shapefile DN-19 Hard
ggplot () +
  geom_sf(data = gdb_dn19_hard, fill = "transparent", color = "black") +
...
shp_water_proj <- shp_water %>%
  sf::st_transform(crs = crs_proj)
gdb_dn19_hard_proj <- gdb_dn19_hard %>%
  sf::st_transform(crs = crs_proj)
...
gdb_dn19_hard_intersect <- sf::st_intersection(
  gdb_dn19_hard_proj,
  selected_region
)
...
p <- ggplot() +
  geom_sf(data = gdb_dn19_hard_intersect, aes(fill = naturtype), color = "black") +
...
ggsave(p, filename=paste0(folder_output_od, "/16. DN_Handbok_19/16.1 DN_Handbok_19/DN_Haandbok_19_Hard", selected_region$vannregion[1], ".png"),
...
gdb_dn19_hard_intersect$naturtype_num <- as.numeric(
  as.factor(gdb_dn19_hard_intersect$naturtype))

# Rasterize the vector data using the unmasked raster as a template
r_hard_dn19 <- terra::rasterize(
  gdb_dn19_hard_intersect,
  r,
  field = "naturtype_num",
  fun = "mean"
)
...
r_hard_dn19_shp <- terra::as.polygons(
  r_hard_dn19, aggregate = TRUE, values = TRUE, field = "naturtype_num")

# Ensure the object is an sf object
r_hard_dn19_shp <- sf::st_as_sf(r_hard_dn19_shp)

# Check if the geometry column is present
print(names(r_hard_dn19_shp))

# Create the plot with color differentiation for different naturtypes
p2 <- ggplot() +
  geom_sf(
  data = r_hard_dn19_shp,
  ...
  labs(
  title = "Rasterized large kelp forest Hard",
...
ggsave(p2, filename=paste0(folder_output_od, "/16. DN_Handbok_19/16.1 DN_Handbok_19/Rasterized DN_Handbok_19_16.1_Hard", selected_region$vannregion[1], ".png"),
     dpi=300, height=20, width=20, units="cm", bg="white")
