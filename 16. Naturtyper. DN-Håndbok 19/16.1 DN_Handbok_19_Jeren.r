...
shp_water_proj <- shp_water %>%
  sf::st_transform(crs = crs_proj)
gdb_dn19_Jeren_proj <- gdb_dn19_Jeren %>%
  sf::st_transform(crs = crs_proj)
...
gdb_dn19_Jeren_intersect <- sf::st_intersection(
  gdb_dn19_Jeren_proj,
  selected_region
)
...
p <- ggplot() +
  geom_sf(data = gdb_dn19_Jeren_intersect, aes(fill = naturtype), color = "black") +
...
ggsave(p, filename=paste0(folder_output_od, "/16. DN_Handbok_19/16.1 DN_Handbok_19/DN_Haandbok_19_Jeren", selected_region$vannregion[1], ".png"),
...
gdb_dn19_Jeren_intersect$naturtype_num <- as.numeric(
  as.factor(gdb_dn19_Jeren_intersect$naturtype))

# Rasterize the vector data using the unmasked raster as a template
r_Jeren_dn19 <- terra::rasterize(
...
r_Jeren_dn19_shp <- terra::as.polygons(
  r_Jeren_dn19, aggregate = TRUE, values = TRUE, field = "naturtype_num")

# Ensure the object is an sf object
r_Jeren_dn19_shp <- sf::st_as_sf(r_Jeren_dn19_shp)

# Check if the geometry column is present
print(names(r_Jeren_dn19_shp))

# Create the plot with color differentiation for different naturtypes
p2 <- ggplot() +
  geom_sf(
  data = r_Jeren_dn19_shp,
...
ggsave(p2, filename=paste0(folder_output_od, "/16. DN_Handbok_19/16.1 DN_Handbok_19/Rasterized DN_Handbok_19_16.1_Jeren", selected_region$vannregion[1], ".png"),
   dpi=300, height=20, width=20, units="cm", bg="white")
