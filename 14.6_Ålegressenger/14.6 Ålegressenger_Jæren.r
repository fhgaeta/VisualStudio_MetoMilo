#plot single area - Jæren 14.7 Tareskog
...
for (i in seq_along(species_list)) {
  ...
  file_out <- paste0(local_folder, "14.7_", species, "_Jæren", ".csv")
...
# load 21 Marine grunnkart -Sårbare habitater - FINNER IKKE TARESKOG I DENNE GDB
...
species_list <- shp_sar_hab %>%
  filter(sarbarthabitat %in% c("XXXX")) %>%
  distinct(sarbarthabitat) %>%
  pull(sarbarthabitat) %>%
  as.character()
