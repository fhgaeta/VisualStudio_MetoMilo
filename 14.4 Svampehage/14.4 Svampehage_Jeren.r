#plot single area - Jæren 14.4 Svampehage
...
species_list <- shp_19_smb %>%
  filter(objtype %in% c("Svampskog", "Svampspikelbunn")) %>%
  distinct(objtype) %>%
  pull(objtype) %>%
  as.character()
...
for (i in seq_along(species_list)) {
  ...
  shp <- shp_19_smb %>% filter(objtype %in% c("Svampskog", "Svampspikelbunn"))
  ...
  file_out <- paste0(local_folder, "14.4_", species, "_Jæren", ".csv")
...
species_list <- shp_sar_hab %>%
  filter(sarbarthabitat %in% c("Svamphage", "Svamphage/Korallskog")) %>%
  distinct(sarbarthabitat) %>%
  pull(sarbarthabitat) %>%
  as.character()
