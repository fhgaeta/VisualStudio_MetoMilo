#plot single area - Jæren 14.5 Sjøfjærbunn
...
for (i in seq_along(species_list)) {
  ...
  file_out <- paste0(local_folder, "14.5_", species, "_Jæren", ".csv")
...
species_list <- shp_sar_hab %>%
  filter(sarbarthabitat %in% c("Sjøfjærbunn", "Cerianthidebunn/Sjøfjærbunn", "Sjøfjærbunn/Svamphage")) %>%
  distinct(sarbarthabitat) %>%
  pull(sarbarthabitat) %>%
  as.character()
