...
shp_hard_low <- read_sf(paste0("C:/Users/", user, "/NIVA/METOMI~1/AP1KAR~1/DATACO~1/",
...
selected_region <- Study_areas_proj %>%
  ...
  shp_hard_low_intersect <- sf::st_intersection(shp_hard_low_proj, selected_region)

  # Plot the intersected shapefile
  ggplot(shp_hard_low_intersect) +
    geom_sf(fill = "#69b3a2", color = "white") +
...
p <- ggplot() +
  ...
  geom_sf(data=shp_hard_low_intersect) +
  coord_sf() +
...
ggsave(p, filename=paste0(folder_output_od, "Low_oksy_Hard_vectorial", selected_region$vannregion[2], ".png"),
...
r_hard_low <- terra::rasterize(shp_hard_low_intersect, r, field=1, fun="mean")

# Convert the rasterized data to polygons for visualization
#rasterized_Hard_low_shp <- as.data.frame(r_salt_low, xy = TRUE)
rasterized_hard_low_shp <- terra::as.polygons(r_hard_low, aggregate = TRUE)

# Ensure the object is an sf object
rasterized_hard_low_shp <- st_as_sf(rasterized_hard_low_shp)

# Check if the geometry column is present
print(names(rasterized_hard_low_shp))

ggplot(rasterized_hard_low_shp) +
...
if (!"geometry" %in% names(rasterized_hard_low_shp)) {
  rasterized_hard_low_shp <- st_as_sf(rasterized_hard_low_shp)
}

# Create the plot
p2 <- ggplot() +
  geom_sf(data = rasterized_hard_low_shp, aes(geometry = geometry), colour = NA, alpha = 0.8, fill = "yellow") +  # Add the rasterized shapefile
...
ggsave(p2, filename=paste0(folder_output_od, "Low_oksygen_Hard_raster_1000", selected_region$vannregion[2], ".png"),
       dpi=300, height=20, width=20, units="cm", bg="white")
