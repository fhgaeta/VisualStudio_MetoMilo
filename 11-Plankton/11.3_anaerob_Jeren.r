#visualize-> Fiskeridir_Fjorder med sjeldan_sørvest_Hardangervidda
...
shp_jer_low <- read_sf(paste0("C:/Users/", user, "/NIVA/METOMI~1/AP1KAR~1/DATACO~1/",
     "GISDAT~1/ECOLOG~1/113E71~1.PLA/119A01~1.3AN/FISKER~1/",
     "Fiskeridir_Fjorder med sjeldan.shp"))
...
shp_jer_low_intersect <- sf::st_intersection(shp_jer_low_proj, selected_region)

# Plot the intersected shapefile
ggplot(shp_jer_low_intersect) +
  geom_sf(fill = "#69b3a2", color = "white") +
...
p <- ggplot() +
  ...
  geom_sf(data=shp_jer_low_intersect) +
  coord_sf() +
...
ggsave(p, filename=paste0(folder_output_od, "Low_oksy_jer_vectorial", selected_region$vannregion[3], ".png"),
...
r_jer_low <- terra::rasterize(shp_jer_low_intersect, r, field=1, fun="mean")

# Convert the rasterized data to polygons for visualization
#rasterized_jer_low_shp <- as.data.frame(r_salt_low, xy = TRUE)
rasterized_jer_low_shp <- terra::as.polygons(r_jer_low, aggregate = TRUE)

# Ensure the object is an sf object
rasterized_jer_low_shp <- st_as_sf(rasterized_jer_low_shp)

# Check if the geometry column is present
print(names(rasterized_jer_low_shp))

ggplot(rasterized_jer_low_shp) +
  geom_raster(aes(x = x, y = y, fill = layer)) +
  scale_fill_viridis_c() +
  theme_minimal() +
  labs(title = "Rasterized Low Oxygen Fjords Hardangervidda", fill = "Value")

# Ensure the geometry column is correctly recognized
if (!"geometry" %in% names(rasterized_jer_low_shp)) {
  rasterized_jer_low_shp <- st_as_sf(rasterized_jer_low_shp)
}

# Create the plot
p2 <- ggplot() +
  geom_sf(data = rasterized_jer_low_shp, aes(geometry = geometry), colour = NA, alpha = 0.8, fill = "yellow") +  # Add the rasterized shapefile
...
ggsave(p2, filename=paste0(folder_output_od, "Low_oksygen_jer_raster_1000", selected_region$vannregion[3], ".png"),
   dpi=300, height=20, width=20, units="cm", bg="white")
