...
shp_beite_jeren <- read_sf("C:/Users/FEG/OneDrive - NIVA/METOMILO_OneDrive/GIS Data/Ecological components data/12. Fisk/12.2 Nursery & feeding areas/12.2 Fiskeridir_Oppvekst beiteområd/Fiskeridir_Oppvekst beiteområd.shp")

# Plot the shapefile
ggplot() +
  geom_sf(data = shp_beite_jeren, fill = "#69b3a2", color = "white") +
...
shp_water_proj <- shp_water %>%
  sf::st_transform(crs=crs_proj)
shp_jeren_beite_proj <- shp_beite_jeren %>%
  sf::st_transform(crs=crs_proj)
...
shp_jeren_beite_intersect <- sf::st_intersection(shp_jeren_beite_proj, selected_region)

# Plot the intersected shapefile
ggplot() +
  geom_sf(data = shp_jeren_beite_intersect, fill = "#69b3a2", color = "white") +
...
ggsave(p, filename=paste0(folder_output_od, "Feeding area_jeren_vectorial", selected_region$vannregion[2], ".png"),
...
r_jeren_beite <- terra::rasterize(shp_jeren_beite_intersect, r, field=1, fun="mean")  

# Convert the rasterized data to polygons for visualization
#rasterized_jeren_low_shp <- as.data.frame(r_jeren_low, xy = TRUE)
rasterized_jeren_beite_shp <- terra::as.polygons(r_jeren_beite, aggregate = TRUE)

# Ensure the object is an sf object
rasterized_jeren_beite_shp <- st_as_sf(rasterized_jeren_beite_shp)

# Check if the geometry column is present
print(names(rasterized_jeren_beite_shp))

# Create the plot with color differentiation for different fish spawning categories
p2 <- ggplot() +
  geom_sf(data = rasterized_jeren_beite_shp, fill = "#1cd85b", color = "white", alpha = 0.8) +
  ...
  labs(title = "Rasterized feeding and nursery areas jeren", fill = "Value") +
  theme_minimal()
...
ggsave(p2, filename=paste0(folder_output_od, "Feeding and nursery area_jeren_1000_raster", selected_region$vannregion[2], ".png"),
       dpi=300, height=20, width=20, units="cm", bg="white")
