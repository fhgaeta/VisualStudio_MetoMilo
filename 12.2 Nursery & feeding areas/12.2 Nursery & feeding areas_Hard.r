...
shp_beite_Hard <- read_sf("C:/Users/FEG/OneDrive - NIVA/METOMILO_OneDrive/GIS Data/Ecological components data/12. Fisk/12.2 Nursery & feeding areas/12.2 Fiskeridir_Oppvekst beiteområd/Fiskeridir_Oppvekst beiteområd.shp")

# Plot the shapefile
ggplot() +
  geom_sf(data = shp_beite_Hard, fill = "#69b3a2", color = "white") +
...
shp_water_proj <- shp_water %>%
  sf::st_transform(crs=crs_proj)
shp_hard_beite_proj <- shp_beite_Hard %>%
  sf::st_transform(crs=crs_proj)
...
selected_region <- Study_areas_proj %>%
  ...
  shp_hard_beite_intersect <- sf::st_intersection(shp_hard_beite_proj, selected_region)

  # Plot the intersected shapefile
  ggplot() +
    geom_sf(data = shp_hard_beite_intersect, fill = "#69b3a2", color = "white") +
...
ggsave(p, filename=paste0(folder_output_od, "Feeding area_Hard_vectorial", selected_region$vannregion[2], ".png"),
...
r_hard_beite <- terra::rasterize(shp_hard_beite_intersect, r, field=1, fun="mean")  

# Convert the rasterized data to polygons for visualization
#rasterized_salt_low_shp <- as.data.frame(r_salt_low, xy = TRUE)
rasterized_hard_beite_shp <- terra::as.polygons(r_hard_beite, aggregate = TRUE)

# Ensure the object is an sf object
rasterized_hard_beite_shp <- st_as_sf(rasterized_hard_beite_shp)


# Check if the geometry column is present
print(names(rasterized_hard_beite_shp))


  # Create the plot with color differentiation for different fish spawning categories
  p2 <- ggplot() +
    geom_sf(data = rasterized_hard_beite_shp, fill = "#1cd85b", color = "white", alpha = 0.8) +
    ...
    labs(title = "Rasterized feeding and nursery areas Hardanger", fill = "Value") +
    theme_minimal()
...
ggsave(p2, filename=paste0(folder_output_od, "Feeding and nursery area_Hardangervidda_1000_raster", selected_region$vannregion[2], ".png"),
       dpi=300, height=20, width=20, units="cm", bg="white")
