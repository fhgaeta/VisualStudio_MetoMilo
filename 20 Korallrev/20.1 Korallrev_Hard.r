...
#read shp file Korallrev Hard
gdb_korallrev_hard <- read_sf(file_path_gdb)

#db_layers <- sf::st_layers(gdb_dn19_hard) 
#db_layers <- db_layers$name

#plot the shapefile Korallrev Hard
ggplot () +
  geom_sf(data = gdb_korallrev_hard, fill = "transparent", color = "black") +
  labs(title = "Korallrev Hard") +
  theme_void()
...
shp_water_proj <- shp_water %>%
  sf::st_transform(crs = crs_proj)
gdb_korallrev_hard_proj <- gdb_korallrev_hard %>%
  sf::st_transform(crs = crs_proj)
...
gdb_korallrev_hard_proj_intersected <- sf::st_intersection(
  gdb_korallrev_hard_proj,
  selected_region
)

# Plot the intersected shapefile categorized by species with different colors
p <- ggplot() +
  geom_sf(data = gdb_korallrev_hard_proj_intersected, aes(fill = naturtypenavn), color = "black") +
...
ggsave(p, filename=paste0(folder_output_od, "/20. Korallrev/Korallrev_vector_Hard", selected_region$vannregion[1], ".png"),
...
gdb_korallrev_hard_proj_intersected$objtype.num <- as.numeric(
  as.factor(gdb_korallrev_hard_proj_intersected$objtype))

# Rasterize the vector data using the unmasked raster as a template
r_hard_korallrev <- terra::rasterize(
  gdb_korallrev_hard_proj_intersected,
  r,
  field = c("objtype.num", "objtype"),
  fun = "mean"
)

# Convert the rasterized data to polygons for visualization
r_hard_korallrev_shp <- terra::as.polygons(
  r_hard_korallrev, aggregate = TRUE, values = TRUE, field = c("objtype.num", "objtype"))

# Ensure the object is an sf object
r_hard_korallrev_shp <- sf::st_as_sf(r_hard_korallrev_shp)

# Check if the geometry column is present
print(names(r_hard_korallrev_shp))

# Create the plot with color differentiation for different naturtypes
p2 <- ggplot() +
  geom_sf(
    data = r_hard_korallrev_shp,
    aes(fill = as.factor(objtype.num_mean)),
    color = NA,
    alpha = 0.8
  ) +
  ...
  labs(
    title = "Rasterized korallrev Hard",
    fill = "Objtype"
  ) +
  theme_minimal()
...
ggsave(p2, filename=paste0(folder_output_od, "/20. Korallrev/Korallrev_Hard_Raster", selected_region$vannregion[1], ".png"),
     dpi=300, height=20, width=20, units="cm", bg="white")
