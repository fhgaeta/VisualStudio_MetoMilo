...
rw_shp_Salten <- readRDS(paste0(folder_output_od, "rw_shp_Salten.rds"))

plot(rw_shp_Salten)

#read shp file Korallrev Salten
gdb_korallrev_Salten <- read_sf(file_path_gdb)

#db_layers <- sf::st_layers(gdb_dn19_Salten) 
#db_layers <- db_layers$name

#plot the shapefile Korallrev Salten
ggplot () +
  geom_sf(data = gdb_korallrev_Salten, fill = "transparent", color = "black") +
  labs(title = "Korallrev Salten") +
  theme_void()
...
shp_water_proj <- shp_water %>%
  sf::st_transform(crs = crs_proj)
gdb_korallrev_Salten_proj <- gdb_korallrev_Salten %>%
  sf::st_transform(crs = crs_proj)
...
gdb_korallrev_Salten_proj_intersected <- sf::st_intersection(
  gdb_korallrev_Salten_proj,
  selected_region
)

# Plot the intersected shapefile categorized by species with different colors
p <- ggplot() +
  geom_sf(data = gdb_korallrev_Salten_proj_intersected, aes(fill = naturtypenavn), color = "black") +
  geom_sf(data = selected_region, fill = "transparent", color = "black") +
  #geom_sf(data = rw_shp_Salten, fill = "transparent", color = "black") +
...
ggsave(p, filename=paste0(folder_output_od, "/20. Korallrev/Korallrev_vector_Salten", selected_region$vannregion[1], ".png"),
...
gdb_korallrev_Salten_proj_intersected$objtype.num <- as.numeric(
  as.factor(gdb_korallrev_Salten_proj_intersected$objtype))

# Rasterize the vector data using the unmasked raster as a template
r_Salten_korallrev <- terra::rasterize(
  gdb_korallrev_Salten_proj_intersected,
  r,
  field = c("objtype.num"),
  fun = "mean"
)
...
r_Salten_korallrev_shp <- terra::as.polygons(
  r_Salten_korallrev, aggregate = TRUE, values = TRUE, field = c("objtype.num"))

# Ensure the object is an sf object
r_Salten_korallrev_shp <- sf::st_as_sf(r_Salten_korallrev_shp)

# Check if the geometry column is present
print(names(r_Salten_korallrev_shp))

# Create the plot with color differentiation for different naturtypes
p2 <- ggplot() +
  geom_sf(
  data = r_Salten_korallrev_shp,
  aes(fill = as.factor(mean)),
  color = NA,
  alpha = 0.8
  ) +
  geom_sf(data = rw_shp_Salten, colour = "grey", fill = NA, alpha = 0.1) +
  ...
  labs(
  title = "Rasterized korallrev Salten",
  fill = "Objtype"
  ) +
  theme_minimal()
...
ggsave(p2, filename=paste0(folder_output_od, "/20. Korallrev/Korallrev_Salten_Raster", selected_region$vannregion[1], ".png"),
   dpi=300, height=20, width=20, units="cm", bg="white")
